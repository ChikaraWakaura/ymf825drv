...
unsigned char readSingle( unsigned char adrs )
{
	unsigned char data;
	ss( LOW );
	bcm2835_spi_transfer( 0x80 | adrs );
...
void writeBurst( unsigned char adrs, unsigned char* data, int count )
{
	...
	ss(LOW);
	bcm2835_spi_transfern( (char *)buf, byte );
	ss(HIGH);
	free( buf );
}
void delayMs( int ms )
...
void initSD1( void )
{
	...
	data = (unsigned char)( ( mastervol << 2 ) & 0xFC );
	writeSingle(25, data);	//	master volume
	writeSingle(27, 0x3f);	//	set itp max
	writeSingle(20, 0x00);	//	set itp on
	writeSingle(3, 0x01);	//	amp gain(6.5[dB])
	data = (unsigned char)( ( ( seqvol << 3 ) & 0xF8 ) | 0x04 );
	writeSingle(9, data);	//	Sequencer Volume , DRV_SV:ON
}
void powerDown( void )
{
	//	1.	damp
