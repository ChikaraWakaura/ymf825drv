...
void Fmvoice_keyon( Fmvoice* _this, void* nt, void* pt, void* tn, unsigned char note, unsigned char vel )
{
	...
	//	Vol
	writeSingle( REG_TOP_ADRS+REG_VOL, ( ( vel / 4 ) << 2 ) & 0x7C );

	//	ChVol
	writeSingle( REG_TOP_ADRS+REG_CH_VOL, Part_cc7( ptp ) );
	...
	//	KeyOn, ToneNum
	writeSingle( REG_TOP_ADRS+REG_CTRL, 0x40 | ( Part_toneNumber(ptp) & 0x0F ) );
}
...
void Fmvoice_damp( Fmvoice* _this )
{
	_this->_keyon = false;
	writeSingle( 11, _this->_vnum );
	writeSingle( REG_TOP_ADRS+REG_CTRL, 0x30 );
...
void Fmvoice_chgChVol( Fmvoice* _this, unsigned char chVol )
{
	chVol = ( ( chVol / 4 ) << 2 | 0x01 ) & 0x7D;
	writeSingle( 11, _this->_vnum );
	writeSingle( REG_TOP_ADRS+REG_CH_VOL, chVol );
}
void Fmvoice_chgExpression( Fmvoice* _this, unsigned char Expression )
{
	Expression = ( ( Expression / 4 ) << 2 | 0x01 ) & 0x7D;
	writeSingle( 11, _this->_vnum );
	writeSingle( REG_TOP_ADRS+REG_CH_VOL, Expression );
}
